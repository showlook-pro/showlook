...
/**
 * Newer Notion responses may wrap records as { value: { value, role } }.
 * Normalize them back to { value, role } so downstream parsing remains stable.
 */
function normalizeRecordMap(recordMap) {
  ...
  unwrap(recordMap.block)
  unwrap(recordMap.collection)
  unwrap(recordMap.notion_user)
  unwrap(recordMap.space)
  return recordMap
}
